...
# Usage: APP_BASE_URL=https://your-app n8n/deploy.sh   (APP_SECRET / GEMINI_API_KEY read from web/.env.local if unset)
...
sed -e "s|APP_SECRET_HERE|$APP_SECRET|" -e "s|GEMINI_API_KEY_HERE|$GEMINI_API_KEY|" n8n/credentials.template.json > "$TMP/credentials.json"
...
docker compose exec -T n8n n8n update:workflow --id=PostpilotGenerate01 --active=true >/dev/null 2>&1
docker compose exec -T -u root n8n rm -rf /tmp/pp-deploy
rm -rf /tmp/pp-deploy
