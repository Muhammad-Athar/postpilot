...
export function MenuContent({ children, align = "end", side = "bottom" }: { children: ReactNode; align?: "start" | "end"; side?: "top" | "bottom" | "left" | "right" }) {
  return (
    <D.Portal>
      <D.Content align={align} side={side} sideOffset={8} collisionPadding={12} asChild>
        <motion.div initial={{ opacity: 0, scale: 0.96, y: side === "top" ? 4 : side === "bottom" ? -4 : 0, x: side === "right" ? -6 : side === "left" ? 6 : 0 }} animate={{ opacity: 1, scale: 1, y: 0, x: 0 }} transition={{ duration: 0.16, ease: [0.22, 1, 0.36, 1] }}
